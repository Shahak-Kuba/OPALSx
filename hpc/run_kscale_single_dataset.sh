...
for ds in "${dataset_list[@]}"; do
    ...
    cmd="${setup_cmds:+$setup_cmds; }cd '$PROJECT_ROOT' && '$JULIA' --project='$HPC_DIR' '$JL_SCRIPT' --dataset='$ds' --k='$k_values' --run='$run_name' 2>&1 | tee '$log'"
    screen -dmS "$session" bash -lc "$cmd"
    echo "▶ launched screen '$session'  (dataset=$ds, k=$k_values)  → log: $log"
done
