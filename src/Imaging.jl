...
Image-loading and mask-building utilities for OPALSx.

This module turns segmented confocal slices into the Boolean `outer`/`inner`
masks consumed by the level-set machinery. Throughout the package, processed
images use a two-channel colour convention:

- **red**   `RGB(1,0,0)` – mineralised matrix outside the Haversian canal (HCa)
- **green** `RGB(0,1,0)` – overlap of HCa and osteonal (On) labels

The *outer* region (cement line) is `red ∪ green`; the *inner* region
(Haversian canal) is the complement of `green`.
"""
module Imaging
...
"""
    generate_RG_img_from_data(path_HCa, path_On, output_path)

Build a two-channel red/green segmentation image from a pair of raw grayscale
slices and write it to `output_path`.

The Haversian-canal (`path_HCa`) and osteonal (`path_On`) images are each
Gaussian-blurred (σ = 2 px) and Otsu-thresholded into binary masks. Their sum
is colourised pixel-wise:

- green `RGB(0,1,0)` where **both** masks are present (overlap), and
- red   `RGB(1,0,0)` where **exactly one** mask is present.

Background pixels stay black. The result is the `Processed_Images` format that
[`build_outer_inner`](@ref) expects.

Arguments
- `path_HCa`    : path to the raw Haversian-canal (HCa) slice
- `path_On`     : path to the raw osteonal (On) slice
- `output_path` : where the colourised PNG/BMP is saved
"""
...
"""
    extract_sample_name(path) -> String

Derive a sample label from an image file `path` by stripping the slice index.

The filename (without directory or extension) is split on `'-'` and the
second-to-last field — the per-slice running number — is dropped, e.g.
`".../FM40-1-R1-On-0042.bmp"` → `"FM40-1-R1-On"`. Used to group the slices of a
stack under a single dataset name.
"""
...
"""
    build_outer_inner(img_paths; downsample=1) -> (outer, inner)

Assemble a stack of red/green segmentation images into the two Boolean volumes
that define the osteon geometry.

`img_paths` is an ordered list of slice paths (one per z-layer, e.g. the sorted
contents of a `Processed_Images` directory). Each slice is loaded, optionally
decimated by `downsample`, and converted to colour masks:

- `outer[:, :, z] = green ∪ red`  — everything inside the cement line, and
- `inner[:, :, z] = ¬green`        — everything outside the Haversian canal.

The zero level-set of `(1-t)·EDT(outer) - t·EDT(inner)` then sweeps from the
canal wall (`t=0`) to the cement line (`t=1`) as formation time `t` increases.

Arguments
- `img_paths`  : ordered vector of slice file paths (length = number of z-layers)

Keyword arguments
- `downsample` : integer in-plane decimation stride (default `1`, i.e. none)
