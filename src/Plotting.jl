...
"""
    plot_3d_surfaces!(ax, ϕ, tvals; dx, dy, dz, σ_μm)

Render the zero-level isosurface of the level-set field `ϕ` for each
formation time in `tvals` using GLMakie's volumetric `contour!`.

The optional `σ_μm` keyword applies an anisotropic Gaussian smooth to each
3-D volume before isosurface extraction, removing the staircase artefacts
caused by the discrete binary-mask origin of the signed distance transform.
...
Keyword arguments
- `dx`, `dy`, `dz` : voxel spacings in µm (used only when σ_μm > 0)
- `σ_μm`           : Gaussian smoothing radius in µm; 0 = no smoothing,
                     ~1 µm gives a clean surface for typical confocal data
"""
function plot_3d_surfaces!(ax, ϕ::AbstractArray{<:Real,4},
    ...
    for ti in eachindex(tvals)
        field = if σ_μm > 0
            smooth_levelset(ϕ[:,:,:,ti]; dx, dy, dz, σ_μm)
        else
            ϕ[:,:,:,ti]
        end
        contour!(ax, 1 .. H, 1 .. W, 1 .. D, field,
                 levels=[0], colormap=:plasma, colorrange=(0,400), isorange=1)
    end
end
